...
describe('Заказ Reducer', () => {
	describe('resetOrderModalData', () => {
		test('Clearing Modal Order', () => {
	...
	describe('fetchOrders', () => {
		...
		test('Выполнен', () => {
			const updated = reducer(
				initialState,
				fetchOrders.fulfilled([mockOrder], 'request-id')
			);
		...
		test('Отклонён', () => {
			...
			const updated = reducer(
				initialState,
				fetchOrders.rejected(err, 'request-id')
			);
			expect(updated.isOrdersLoading).toBe(false);
	...
	describe('fetch_Order', () => {
		test('pending', () => {
			const updated = reducer(
				initialState,
				fetchOrder.pending('req-id', mockOrder.number)
			);
			expect(updated.isOrderLoading).toBe(true);
		});
		...
		test('Отклонён', () => {
			...
			const updated = reducer(
				initialState,
				fetchOrder.rejected(err, 'req-id', mockOrder.number)
			);
			expect(updated.isOrderLoading).toBe(false);
		});
	});

	describe('Содание заказа', () => {
		test('Ожидание', () => {
			const updated = reducer(
				initialState,
				createOrder.pending('req-id', mockOrder.ingredients)
			);
		...
		test('Выполнен', () => {
			...
			const updated = reducer(
				initialState,
				createOrder.fulfilled(payload, 'req-id', mockOrder.ingredients)
			);
			expect(updated.orderRequest).toBe(false);
			expect(updated.orderModalData).toEqual(mockOrder);
		});
		...
		test('Отклонён', () => {
			...
			const updated = reducer(
				initialState,
				createOrder.rejected(err, 'req-id', [])
			);
			expect(updated.orderRequest).toBe(false);
		});
	});
